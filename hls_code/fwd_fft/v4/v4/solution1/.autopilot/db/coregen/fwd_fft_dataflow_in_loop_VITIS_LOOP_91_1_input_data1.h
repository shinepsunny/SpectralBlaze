...
#ifndef _fwd_fft_dataflow_in_loop_VITIS_LOOP_91_1_input_data1_H_
#define _fwd_fft_dataflow_in_loop_VITIS_LOOP_91_1_input_data1_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "fwd_fft_dataflow_in_loop_VITIS_LOOP_91_1_input_data1_memcore.h"

SC_MODULE(fwd_fft_dataflow_in_loop_VITIS_LOOP_91_1_input_data1) {
...
protected:
    ...
    // buffer instances
    fwd_fft_dataflow_in_loop_VITIS_LOOP_91_1_input_data1_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(fwd_fft_dataflow_in_loop_VITIS_LOOP_91_1_input_data1) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "fwd_fft_dataflow_in_loop_VITIS_LOOP_91_1_input_data1_memcore_U_";
            name += szTmp;

            buffer[i] = new fwd_fft_dataflow_in_loop_VITIS_LOOP_91_1_input_data1_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "fwd_fft_dataflow_in_loop_VITIS_LOOP_91_1_input_data1.trace");
            // ports
