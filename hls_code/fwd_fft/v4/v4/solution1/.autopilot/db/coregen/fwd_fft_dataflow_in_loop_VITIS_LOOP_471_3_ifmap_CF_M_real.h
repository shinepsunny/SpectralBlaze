...
#ifndef _fwd_fft_dataflow_in_loop_VITIS_LOOP_471_3_ifmap_CF_M_real_H_
#define _fwd_fft_dataflow_in_loop_VITIS_LOOP_471_3_ifmap_CF_M_real_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "fwd_fft_dataflow_in_loop_VITIS_LOOP_471_3_ifmap_CF_M_real_memcore.h"

SC_MODULE(fwd_fft_dataflow_in_loop_VITIS_LOOP_471_3_ifmap_CF_M_real) {
    static const unsigned int DataWidth    = 32;
    static const unsigned int EnableWidth    = 4;
...
protected:
    ...
    // buffer instances
    fwd_fft_dataflow_in_loop_VITIS_LOOP_471_3_ifmap_CF_M_real_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(fwd_fft_dataflow_in_loop_VITIS_LOOP_471_3_ifmap_CF_M_real) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "fwd_fft_dataflow_in_loop_VITIS_LOOP_471_3_ifmap_CF_M_real_memcore_U_";
            name += szTmp;

            buffer[i] = new fwd_fft_dataflow_in_loop_VITIS_LOOP_471_3_ifmap_CF_M_real_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "fwd_fft_dataflow_in_loop_VITIS_LOOP_471_3_ifmap_CF_M_real.trace");
            // ports
