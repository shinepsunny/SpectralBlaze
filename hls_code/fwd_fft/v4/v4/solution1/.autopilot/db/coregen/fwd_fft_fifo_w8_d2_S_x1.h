...
// ==============================================================
#ifndef fwd_fft_fifo_w8_d2_S_x1_HH_
#define fwd_fft_fifo_w8_d2_S_x1_HH_

#include <systemc>
using namespace std;
SC_MODULE(fwd_fft_fifo_w8_d2_S_x1) {
    static const unsigned int DATA_WIDTH = 8;
    static const unsigned int ADDR_WIDTH = 2;
    static const unsigned int fwd_fft_fifo_w8_d2_S_x1_depth = 3;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[fwd_fft_fifo_w8_d2_S_x1_depth];
    ...
    SC_CTOR(fwd_fft_fifo_w8_d2_S_x1) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < fwd_fft_fifo_w8_d2_S_x1_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (fwd_fft_fifo_w8_d2_S_x1_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < fwd_fft_fifo_w8_d2_S_x1_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (fwd_fft_fifo_w8_d2_S_x1_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > fwd_fft_fifo_w8_d2_S_x1_depth) {
...
#endif //fwd_fft_fifo_w8_d2_S_x1_HH_
