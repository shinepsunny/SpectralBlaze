...
#(parameter
    ...
    output wire [31:0]                   ctrl1_regp,
    output wire [31:0]                   ctrl2_regp,
    output wire [31:0]                   layer1_regp,
    output wire [31:0]                   layer2_regp,
    output wire [31:0]                   actp_regp,
...
localparam
    ...
    ADDR_OUT1_DATA_0        = 7'h1c,
    ADDR_OUT1_DATA_1        = 7'h20,
    ADDR_OUT1_CTRL          = 7'h24,
    ADDR_OUT2_DATA_0        = 7'h28,
    ADDR_OUT2_DATA_1        = 7'h2c,
    ADDR_OUT2_CTRL          = 7'h30,
    ADDR_CTRL1_REGP_DATA_0  = 7'h34,
    ADDR_CTRL1_REGP_CTRL    = 7'h38,
    ADDR_CTRL2_REGP_DATA_0  = 7'h3c,
    ADDR_CTRL2_REGP_CTRL    = 7'h40,
    ADDR_LAYER1_REGP_DATA_0 = 7'h44,
    ADDR_LAYER1_REGP_CTRL   = 7'h48,
    ADDR_LAYER2_REGP_DATA_0 = 7'h4c,
    ADDR_LAYER2_REGP_CTRL   = 7'h50,
    ADDR_ACTP_REGP_DATA_0   = 7'h54,
    ADDR_ACTP_REGP_CTRL     = 7'h58,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_ctrl1_regp = 'b0;
    reg  [31:0]                   int_ctrl2_regp = 'b0;
    reg  [31:0]                   int_layer1_regp = 'b0;
    reg  [31:0]                   int_layer2_regp = 'b0;
    reg  [31:0]                   int_actp_regp = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_CTRL1_REGP_DATA_0: begin
                    rdata <= int_ctrl1_regp[31:0];
                end
                ADDR_CTRL2_REGP_DATA_0: begin
                    rdata <= int_ctrl2_regp[31:0];
                end
                ADDR_LAYER1_REGP_DATA_0: begin
                    rdata <= int_layer1_regp[31:0];
                end
                ADDR_LAYER2_REGP_DATA_0: begin
                    rdata <= int_layer2_regp[31:0];
                end
                ADDR_ACTP_REGP_DATA_0: begin
                    rdata <= int_actp_regp[31:0];
                end
            endcase
        end
    end
...
assign ctrl1_regp    = int_ctrl1_regp;
assign ctrl2_regp    = int_ctrl2_regp;
assign layer1_regp   = int_layer1_regp;
assign layer2_regp   = int_layer2_regp;
assign actp_regp     = int_actp_regp;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl1_regp[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL1_REGP_DATA_0)
            int_ctrl1_regp[31:0] <= (WDATA[31:0] & wmask) | (int_ctrl1_regp[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl2_regp[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL2_REGP_DATA_0)
            int_ctrl2_regp[31:0] <= (WDATA[31:0] & wmask) | (int_ctrl2_regp[31:0] & ~wmask);
    end
end

// int_layer1_regp[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_layer1_regp[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_LAYER1_REGP_DATA_0)
            int_layer1_regp[31:0] <= (WDATA[31:0] & wmask) | (int_layer1_regp[31:0] & ~wmask);
    end
end

// int_layer2_regp[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_layer2_regp[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_LAYER2_REGP_DATA_0)
            int_layer2_regp[31:0] <= (WDATA[31:0] & wmask) | (int_layer2_regp[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_actp_regp[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_ACTP_REGP_DATA_0)
            int_actp_regp[31:0] <= (WDATA[31:0] & wmask) | (int_actp_regp[31:0] & ~wmask);
    end
end
