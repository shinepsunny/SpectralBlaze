...
typedef struct {
    ...
    XIfft_ap_uio_map maps[ MAX_UIO_MAPS ];
} XIfft_ap_uio_info;

/***************** Variable Definitions **************************************/
static XIfft_ap_uio_info uio_info;
...
static int uio_info_read_name(XIfft_ap_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XIfft_ap_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XIfft_ap_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XIfft_ap_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XIfft_ap_Initialize(XIfft_ap *InstancePtr, const char* InstanceName) {
	XIfft_ap_uio_info *InfoPtr = &uio_info;
